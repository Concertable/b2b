...
| Tenant-independent read, `SaveChanges` throws | `ReadDbContext` (shared DataAccess) | `Application`, `Artist`, `Booking`, `Concert`, `Conversations`, `Opportunity`, `Venue` |
| Unscoped but writable | `PrivilegedDbContext` | `ConversationsPrivilegedDbContext` (moderation, GDPR erasure) |
| Untenanted module | `DbContextBase` + own `OnModelCreating` | `Admin`, `Deal`, `Privacy`, `Tenant`, `User` — no base owns their `OnModelCreating`; `api/TECH_DEBT.md` holds the repo-wide entry |
