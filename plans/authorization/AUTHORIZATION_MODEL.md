...
| Commands | There are real authorization functions outside filters: `HasPermission`, Concert's `IsPrincipal` and grant mutation guards, and `ConversationEntity.Admits`. Privileged ACL repository methods exist. What is missing is a consistently consumed, resource-aware decision/fence boundary. [Concert domain][C15], [privileged repository][C16], [conversation][C17] |
...
| ConversationAccessGrant | Read, SendMessages | ConversationsDbContext |
...
| R22: Staff messaging cannot be exercised | Confirmed at the reviewed baseline. Staff receives `messages.read/send` with AssignedResources, while conversation creation issues tenant-wide principal grants. The delivered conversation member-assignment path now issues explicit Read and SendMessages grants to a current membership. Reachability qualification remains part of the wider catalog work. [Catalog][C6], [conversation issuance][C17], [Concert assignment][C15] |
...
- ACL administrators can find their issued grants, revoke/reissue them and replay safely; recipients cannot obtain or administer the full ACL. Authorized conversation participant queries remain complete without disclosing unrelated ACL details.
...
[C17]: ../party-foundation/PARTY_FOUNDATION_PLAN.md#47-conversations-have-an-identity-an-audience-and-a-monotonic-read-position
