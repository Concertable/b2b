...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    server: {
      host: '127.0.0.1',
      https: command === 'serve'
        ? aspNetDevelopmentHttps(path.resolve(__dirname, '../../../node_modules/.vite/aspnet-https/venue'))
        : undefined,
      port: 5175,
    },
    envDir: '../',
    define: command === 'build'
      ...
      : {
          'import.meta.env.VITE_AUTH_AUTHORITY': JSON.stringify('http://localhost:7083'),
          'import.meta.env.VITE_OIDC_CLIENT_ID': JSON.stringify('venue-web'),
          'import.meta.env.VITE_OIDC_SCOPE': JSON.stringify('openid profile concertable.b2b.api offline_access'),
