...
internal sealed class ApplicationDealStrategyBuilder
{
    ...
    public ApplicationDealStrategyBuilder(IServiceCollection services)
    {
        this.services = services;
    }

    public ApplicationDealTypeStrategyBuilder For(DealType dealType)
    {
        if (!Enum.IsDefined(dealType))
            throw new InvalidOperationException($"{dealType} is not a declared deal type.");

        return new ApplicationDealTypeStrategyBuilder(this, dealType);
    }

    public ApplicationDealStrategyBuilder RequireAll<TStrategy>()
        where TStrategy : class =>
        RequireExactly<TStrategy>(Enum.GetValues<DealType>());

    public ApplicationDealStrategyBuilder RequireExactly<TStrategy>(params DealType[] dealTypes)
        where TStrategy : class
    {
        ...
        where TImplementation : class, TStrategy
    {
        ...
        Action<IServiceCollection> add = lifetime switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"{lifetime} is not a supported Application deal strategy lifetime.")
        };
...
internal sealed class ApplicationDealTypeStrategyBuilder
{
    private readonly ApplicationDealStrategyBuilder builder;
    private readonly DealType dealType;

    public ApplicationDealTypeStrategyBuilder(ApplicationDealStrategyBuilder builder, DealType dealType)
    {
        this.builder = builder;
        this.dealType = dealType;
    }

    public ApplicationDealTypeStrategyBuilder AddSingleton<TStrategy, TImplementation>()
        where TStrategy : class
        where TImplementation : class, TStrategy
    {
    ...
    public ApplicationDealTypeStrategyBuilder AddScoped<TStrategy, TImplementation>()
        where TStrategy : class
        where TImplementation : class, TStrategy
    {
