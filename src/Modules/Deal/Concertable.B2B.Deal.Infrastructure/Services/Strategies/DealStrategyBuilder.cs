...
internal sealed class DealStrategyBuilder
{
    ...
    public DealTypeStrategyBuilder For(DealType dealType)
    {
        if (!Enum.IsDefined(dealType))
            throw new InvalidOperationException($"{dealType} is not a declared deal type.");

        return new DealTypeStrategyBuilder(this, dealType);
    }

    public DealStrategyBuilder RequireAll<TStrategy>()
        ...
        where TImplementation : class, TStrategy
    {
        ...
        Action<IServiceCollection> add = lifetime switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"{lifetime} is not a supported Deal strategy lifetime.")
        };
...
internal sealed class DealTypeStrategyBuilder
{
    private readonly DealStrategyBuilder builder;
    private readonly DealType dealType;

    public DealTypeStrategyBuilder(DealStrategyBuilder builder, DealType dealType)
    {
        this.builder = builder;
        this.dealType = dealType;
    }

    public DealTypeStrategyBuilder AddSingleton<TStrategy, TImplementation>()
        where TStrategy : class
        where TImplementation : class, TStrategy
    {
    ...
    public DealTypeStrategyBuilder AddScoped<TStrategy, TImplementation>()
        where TStrategy : class
        where TImplementation : class, TStrategy
    {
