...
internal interface IUserRepository : IRepository<UserEntity, Guid>
{
    ...
    /// <summary>A single tracked invitation to mutate (revoke); null if it doesn't exist.</summary>
    Task<AdminInvitationEntity?> GetInvitationByIdAsync(Guid id, CancellationToken ct = default);

    void AddInvitation(AdminInvitationEntity invitation);
}
