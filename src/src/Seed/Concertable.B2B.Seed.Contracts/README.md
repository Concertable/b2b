...
Pure data, no behaviour. References only the three B2B `*.Contracts` projects and `Concertable.Seed.Identity`. Zero runtime / EF / Domain dependencies — ships as a private NuGet package in the split-repo future.

## Who consumes it

- `Concertable.B2B.Seed.Infrastructure` — real B2B's seeders project from these lists into Domain entities + publish the events.
- `Concertable.B2B.Seed.Simulator` — standalone Worker that publishes these events when real B2B isn't running (consumed by `Concertable.Customer.AppHost` for standalone dev).
